    ...
    override suspend fun decryptMessage(topic: String, message: String, onSuccess: (NotifyMessage) -> Unit, onFailure: (Throwable) -> Unit) = supervisorScope {
        try {
            ...
            val clientJsonRpc = serializer.tryDeserialize<ClientJsonRpc>(decryptedMessageString)
                ?: return@supervisorScope onFailure(IllegalArgumentException("The decrypted message does not match WalletConnect JSON-RPC format"))
            val notifyMessage = serializer.deserialize(clientJsonRpc.method, decryptedMessageString)
            // TODO: Will need to decode JWT to get message
//            val notifyMessageEngineDO = NotifyParams.MessageParams::class.safeCast(notifyMessage)?.toEngineDO()
