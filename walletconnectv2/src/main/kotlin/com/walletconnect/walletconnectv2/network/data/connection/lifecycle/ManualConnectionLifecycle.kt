...
internal class ManualConnectionLifecycle(
    ...
    init {
        if (connectionController is ConnectionController.Manual) {
            connectionController.connectionStateFlow
                .onEach { state ->
                    when (state) {
                        ConnectionState.CONNECT -> lifecycleRegistry.onNext(Lifecycle.State.Started)
                        ConnectionState.DISCONNECT -> lifecycleRegistry.onNext(Lifecycle.State.Stopped.WithReason())
                    }
                }
                .launchIn(scope)
        }
