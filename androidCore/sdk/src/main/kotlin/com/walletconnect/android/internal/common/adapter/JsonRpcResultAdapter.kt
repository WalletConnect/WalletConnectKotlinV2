...
internal class JsonRpcResultAdapter(val moshi: Moshi) : JsonAdapter<JsonRpcResponse.JsonRpcResult>() {
    ...
    private val approvalParamsAdapter: JsonAdapter<CoreSignParams.ApprovalParams> = moshi.adapter(CoreSignParams.ApprovalParams::class.java, emptySet(), "result")
    private val cacaoAdapter: JsonAdapter<CoreAuthParams.ResponseParams> = moshi.adapter(CoreAuthParams.ResponseParams::class.java, emptySet(), "result")
    private val acceptanceParamsAdapter: JsonAdapter<CoreChatParams.AcceptanceParams> = moshi.adapter(CoreChatParams.AcceptanceParams::class.java, emptySet(), "result")
            ...
            when (reader.selectName(options)) {
                ...
                2 -> {
                    result = when {
                        ...
                        runCatching { cacaoAdapter.fromJson(reader.peekJson()) }.isSuccess -> cacaoAdapter.fromJson(reader)
                        runCatching { acceptanceParamsAdapter.fromJson(reader.peekJson()) }.isSuccess -> acceptanceParamsAdapter.fromJson(reader)
    ...
    override fun toJson(writer: JsonWriter, value_: JsonRpcResponse.JsonRpcResult?) {
        ...
        when {
            ...
            (value_.result as? PushParams.RequestParams) != null -> {
                val requestParams = requestParamsAdapter.toJson(value_.result)
                writer.valueSink().use {
            ...
            (value_.result as? PushParams.MessageParams) != null -> {
                val messageParams = requestMessageParamsAdapter.toJson(value_.result)
                writer.valueSink().use {
            ...
            (value_.result as? PushParams.RequestResponseParams) != null -> {
                val requestResponseParamsString = requestResponseParamsAdapter.toJson(value_.result)
                writer.valueSink().use {
