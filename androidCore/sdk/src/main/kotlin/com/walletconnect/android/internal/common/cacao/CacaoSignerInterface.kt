...
inline fun <CoreSignature : SignatureInterface, reified SDKSignature : CoreSignature> CacaoSignerInterface<CoreSignature>.sign(message: String, privateKey: ByteArray, type: ISignatureType):
        CoreSignature = when (type.header) {
    SignatureType.EIP191.header, SignatureType.EIP1271.header -> SDKSignature::class.cast(Cacao.Signature(type.header, EIP191Signer.sign(message.toByteArray(), privateKey).toCacaoSignature()))
