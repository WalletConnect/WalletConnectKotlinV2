...
internal class OnSyncUpdateEventUseCase(
    ...
    private val onChatReceivedInviteStatusUpdateEventUseCase: OnChatReceivedInviteStatusUpdateEventUseCase,
) {
    suspend operator fun invoke(event: Events.OnSyncUpdate) {
        when (event.store.value) {
            ...
            CHAT_RECEIVED_INVITE_STATUSES.value -> onChatReceivedInviteStatusUpdateEventUseCase(event)
        }
    }
}
